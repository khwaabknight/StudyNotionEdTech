...
export function signUp(accountType,firstName,lastName,email,password,confirmPassword,otp,navigate) {
    return async (dispatch) => {
        ...
        try {
            const response = await apiConnector("POST",SIGNUP_API,requestObject)
...
export function getPasswordResetToken(email,setEmailSent) {
    return async (dispatch) => {
        ...
        try {
            const response = await apiConnector("POST", RESET_PASS_TOKEN_API, {email})
...
export function resetPassword(password,confirmPassword,token,strongPasswordcharacters,navigate) {
    return async (dispatch) => {
        const toastId = toast.loading("Loading...")
        dispatch(setLoading(true))
        try {
            const {lowercase,special,uppercase,length,number} = strongPasswordcharacters;
            if(!lowercase && !special && !uppercase && !number && (length<8)) {
